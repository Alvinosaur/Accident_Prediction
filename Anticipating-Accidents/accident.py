...
def build_model():
    ...
    lstm_cell = tf.contrib.rnn.LSTMCell(n_hidden, initializer=tf.random_normal_initializer(
        mean=0.0, stddev=0.01), use_peepholes=True, state_is_tuple=False)
    ...
    loss = 0.0
    # Mask
    # tf.reduce_sum(transpose(all of x, [1,2,0,3])) just squashes all 4096 input
    # features into one value
    zeros_object = tf.to_float(tf.not_equal(
        tf.reduce_sum(
            # frames x n_detection-1 x batches x n_input
            # why skip the 0th detection
            tf.transpose(x[:, :, 1:n_detection, :], [1, 2, 0, 3]),
            3),
        0))  # frame x n x b
    # Start creat graph
    for i in range(n_frames):
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
            ...
            n_object = tf.matmul(
                n_object, weights['em_obj']) + biases['em_obj']  # (n x b) x h
            n_object = tf.reshape(
                n_object, [n_detection - 1, batch_size, n_att_hidden])  # n-1 x b x h
            n_object = tf.multiply(
                n_object, tf.expand_dims(zeros_object[i], 2))

            # object attention
            brcst_w = tf.tile(tf.expand_dims(weights['att_w'], 0), [
                              n_detection - 1, 1, 1])  # n x h x 1
            image_part = tf.matmul(n_object, tf.tile(tf.expand_dims(weights['att_ua'], 0), [
                                   n_detection - 1, 1, 1])) + biases['att_ba']  # n x b x h
            # n x b x h
            ...
            h_prev = outputs
            # FC to output
            pred = tf.matmul(outputs, weights['out']) + \
                biases['out']  # b x n_classes
            ...
            neg_loss = tf.nn.softmax_cross_entropy_with_logits(
                labels=y, logits=pred)  # Softmax loss

            temp_loss = tf.reduce_mean(
                tf.add(tf.multiply(pos_loss, y[:, 1]), tf.multiply(neg_loss, y[:, 0])))
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print("Training")
            test_all(sess, train_num, train_path, x, keep,
                     y, loss, lstm_variables, soft_pred)
            print("Testing")
            test_all(sess, test_num, test_path, x, keep,
                     y, loss, lstm_variables, soft_pred)
...
def test_all(sess, num, path, x, keep, y, loss, lstm_variables, soft_pred):
    ...
    for num_batch in range(1, num + 1):
        ...
        [temp_loss, pred] = sess.run([loss, soft_pred], feed_dict={
                                     x: test_data, y: test_labels, keep: [0.0]})
...
def evaluation(all_pred, all_labels, total_time=90, vis=False, length=None):
    ...
    if length is not None:
        ...
        for idx, vid in enumerate(length):
            all_pred_tmp[idx, total_time -
                         vid:] = all_pred[idx, total_time - vid:]
    ...
    for i in range(1, len(new_Precision)):
        AP += (new_Precision[i - 1] + new_Precision[i]) * \
            (new_Recall[i] - new_Recall[i - 1]) / 2

    print("Average Precision= " + "{:.4f}".format(AP) +
          " ,mean Time to accident= " + "{:.4}".format(np.mean(new_Time) * 5))
    sort_time = new_Time[np.argsort(new_Recall)]
    sort_recall = np.sort(new_Recall)
    print("Recall@80%, Time to accident= " +
          "{:.4}".format(sort_time[np.argmin(np.abs(sort_recall - 0.8))] * 5))
...
def vis(model_path):
    ...
    for num_batch in range(1, test_num):
        ...
        ID = all_data['ID']
        # run result
        [all_loss, pred, weight] = sess.run([loss, soft_pred, all_alphas], feed_dict={
                                            x: data, y: labels, keep: [0.0]})
                    ...
                    for num_box in index:
                        if now_weight[num_box] / 255.0 > 0.4:
                            cv2.rectangle(frame, (new_bboxes[num_box, 0], new_bboxes[num_box, 1]), (
                                new_bboxes[num_box, 2], new_bboxes[num_box, 3]), (0, 255, 0), 3)
                        else:
                            cv2.rectangle(frame, (new_bboxes[num_box, 0], new_bboxes[num_box, 1]), (
                                new_bboxes[num_box, 2], new_bboxes[num_box, 3]), (255, 0, 0), 2)
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(frame, str(round(now_weight[num_box] / 255.0 * 10000) / 10000), (
                            new_bboxes[num_box, 0], new_bboxes[num_box, 1]), font, 0.5, (0, 0, 255), 1, cv2.CV_AA)
                        attention_frame[int(new_bboxes[num_box, 1]):int(new_bboxes[num_box, 3]), int(
                            new_bboxes[num_box, 0]):int(new_bboxes[num_box, 2])] = now_weight[num_box]

                    attention_frame = cv2.applyColorMap(
                        attention_frame, cv2.COLORMAP_HOT)
                    dst = cv2.addWeighted(frame, 0.6, attention_frame, 0.4, 0)
                    cv2.putText(dst, str(counter + 1), (10, 30),
